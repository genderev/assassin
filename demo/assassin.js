...
class Assassin {
    ...
    // let's parse the object into JSON within array
    const res = JSON.parse(xhr.responseText);
    //send complete object to main thread. deep clone the object
    self.postMessage(JSON.parse(JSON.stringify(res)))
    // add object keys and object values into tree to create tree
    tree.load(Object.keys(res),Object.values(res))
    ...
    update(k,val){
      ...
      tree.insert(k,val)
      const base = {}
      tree.keys().forEach((key, i) => base[key] = tree.values()[i]);
      self.postMessage(base)
      var xhr = new XMLHttpRequest();xhr.open("POST", write, true);
      xhr.send(JSON.stringify(base));
    }
    ...
    delete(key){
      ...
      tree.keys().forEach((key, i) => base[key] = tree.values()[i]);
      self.postMessage(base)
      const xhr = new XMLHttpRequest(); xhr.open("POST", write , true);
      xhr.send(JSON.stringify(base));
    }

    create (k,val){
      if (tree.contains(k)==false){
        tree.insert(k,val);
        const base = {};
        tree.keys().forEach((key, i) => base[key] = tree.values()[i]);
        self.postMessage(base);
